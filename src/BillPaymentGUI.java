...
public class BillPaymentGUI extends JFrame {
    ...
    public BillPaymentGUI(TransactionManager manager, BankAppGUI mainWindow) {
        ...
        JLabel titleLBL = new JLabel("Bills Payment", SwingConstants.CENTER);
        titleLBL.setBounds(0, 45, 390, 35);
        titleLBL.setFont(new Font("Arial", Font.BOLD, 24));
        titleLBL.setForeground(Color.WHITE);
        headerPanel.add(titleLBL);

        JLabel subTitleLBL = new JLabel("Select biller and enter amount", SwingConstants.CENTER);
        ...
        billLBL.setForeground(new Color(30, 50, 100));
        billLBL.setBounds(30, 145, 330, 25);
        add(billLBL);

        JPanel billerPanel = new JPanel(null);
        billerPanel.setBounds(30, 173, 330, 165);
        billerPanel.setBackground(Color.WHITE);
        billerPanel.setBorder(BorderFactory.createLineBorder(new Color(30, 50, 100), 1));
        add(billerPanel);

        meralco = makeRadio("Meralco",0);
        pldt = makeRadio("PLDT",1);
        globe = makeRadio("Globe",2);
        maynilad = makeRadio("Maynilad",3);
        skycable = makeRadio("Sky Cable",4);

        billerPanel.add(meralco);
        billerPanel.add(pldt);
        billerPanel.add(globe);
        billerPanel.add(maynilad);
        billerPanel.add(skycable);
        ...
        subBTN.addActionListener(e -> {
            ...
            if (selectedBiller.isEmpty()) {
                JOptionPane.showMessageDialog(this, "Please select a biller.");
                return;
            }
            ...
            try {
                ...
                String result = manager.addTransaction("Bills Payment", amount, date, selectedBiller);
                if (result.equals("SUCCESS")) {
                    JOptionPane.showMessageDialog(this, "Payment to " + selectedBiller + " successful!");
