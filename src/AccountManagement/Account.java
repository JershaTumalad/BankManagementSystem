...
public class Account {
    ...
    public void setAccountNo(String accountNo) {
        if(accountNo != null && !accountNo.trim().isEmpty()) this.accountNo = accountNo.trim();
    }
    public void setName(String name) {
        if(name != null && !name.trim().isEmpty()) this.name = name.trim();
    }
    public void setAccountType(String accountType) {
        if(accountType != null && !accountType.trim().isEmpty()) this.accountType = accountType.trim();
    }
    ...
    public static boolean validateUniqueAccount(String accNo, ArrayList<Account> accounts){
        for(Account acc : accounts){
            if(acc.getAccountNo().equals(accNo.trim())) return false;
        }
        return true;
    }
